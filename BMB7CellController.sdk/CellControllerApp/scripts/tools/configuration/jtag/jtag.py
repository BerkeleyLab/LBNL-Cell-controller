...
class chain(states):
    ...
    def __update_configuration(self):
        ...
        if self.__mode_two_bit == True:
            if self.__burst_size > 4:
                raise EthernetToJTAGException('Burst size greater than 4 is not allowed in TMS/TDI mode')
        else:
            if self.__burst_size > 8:
                raise EthernetToJTAGException('Burst size greater than 8 is not allowed in TDI-only mode')
        ...
        self.__config = bytearray([
            token,
            self.__speed,
            (self.__bit_in_select << 4) | self.__channel_enable,
            (self.__bit_in_phase << 4) | (self.__mode_write_endian << 3) | (self.__mode_read_endian << 2) | (self.__mode_two_bit << 1) | self.__mode_rw
        ])
    ...
    # Just read from JTAG
    def read(self, num_bits, update_register = False, flush_first_character=False, reverse=False):
    ...
    # Just write to JTAG
    def write_bytearray(self, data, update_register = False, endian = False, flush_first_character = False):
        if ( (self.state != states.SHIFT_IR) and
             (self.state != states.SHIFT_DR) ):
            raise EthernetToJTAGException('Invalid state for this instruction')
    ...
    def write_read_bytearray(self,  data, update_register = False, write_endian = False, read_endian = False, flush_first_character = False):
        if ( (self.state != states.SHIFT_IR) and
             (self.state != states.SHIFT_DR) ):
            raise EthernetToJTAGException('Invalid state for this instruction')
        
        if (update_register or flush_first_character) == False:
            return self.jtag_clock_with_result_burst_8(data, write_endian, read_endian)
    ...
    # Just write to JTAG
    def write(self, data, num_bits, update_register = False, flush_first_character = False, endian = False):
        if ( (self.state != states.SHIFT_IR) and
             (self.state != states.SHIFT_DR) ):
            raise EthernetToJTAGException('Invalid state for this instruction')
        if ( num_bits == 0 ):
    ...
    # Write to JTAG and read result at the same time
    def write_read(self, data, num_bits, update_register = False, reverse = False, flush_first_character = False):
        if ( (self.state != states.SHIFT_IR) and
             (self.state != states.SHIFT_DR) ):
            raise EthernetToJTAGException('Invalid state for this instruction')
        if ( num_bits == 0 ):
    ...
    def jtag_clock_burst_8(self, data, endian):
        ...
        if (self.__mode_two_bit == True) or (self.__burst_size != 8) or (self.__mode_write_endian != endian):
    ...
    def jtag_clock_with_result_burst_8(self, data, write_endian, read_endian):
        ...
        # of bits from a single command and processes it
        # It returns the resulting packet
        if (self.__mode_two_bit == True) or (self.__burst_size != 8) or (self.__mode_write_endian != write_endian) or (self.__mode_read_endian != read_endian):
            self.__mode_two_bit = False
            self.__burst_size = 8
    ...
    def jtag_exec(self, data):
        ...
        clocks_per_mtu = max_cycles_per_mtu * clocks_per_cycle + 10 + (self.__speed + 1)
